          ...
          {Navigation.map((item) => {
            return (
              <Link
                key={item.name}
                to={item.path}
                className={`${selectedOption === item.name ? "bg-white/30 text-gray-800" : ""} m-2 p-2 px-4 font-medium rounded-3xl text-gray-200 hover:text-gray-800 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-transparent`}
              >
                {item.name}
              </Link>
            );
